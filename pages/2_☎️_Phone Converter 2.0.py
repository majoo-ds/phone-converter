...
upload_file =  st.file_uploader("Upload your Excel file", help="Please upload your file at first to prevent errors.")
...
if upload_file is not None:
    ...
    st.write(f"You selected {select_col}")

    # slicing new dataframe (phone_selected only)
    df_origin = df.loc[:, [select_col]].copy()
    df_origin.columns = ["origin_phone_number"]
    df_origin["origin_phone_number"] = df_origin["origin_phone_number"].astype("str")
    ...
    merge_prefix_df = pd.merge(df, prefix_df, how="left", left_on="prefix", right_on="prefix")

    # merge df with origin
    merge_origin_df = pd.merge(df_origin, merge_prefix_df, how="outer", left_on="origin_phone_number", right_on=select_col)

    # tagging clean and unclean (new column)
    merge_origin_df["clened_or_uncleaned"] = merge_origin_df.apply(
        lambda row:
        "uncleaned" if pd.isnull(row["nomor_telepon_clean"])
        else "cleaned", axis=1
    )

    st.markdown("### Cleaned File Result")


    # st metrics
    len_origin = len(df_origin)
    len_cleaned = len(merge_origin_df.loc[merge_origin_df["nomor_telepon_clean"].notnull()])
    len_uncleaned = len_origin - len_cleaned
    col1, col2, col3, col4 = st.columns(4)
    col1.metric(label="Num of Origins", value=len_origin)
    col2.metric(label="Num of Cleaned", value=len_cleaned)
    col3.metric(label="Num of Uncleaned", value=len_uncleaned)
    col4.metric(label="Cleaning Rate", value="{:.2%}".format(len_cleaned/len_origin))

    # AG Grid Dataframe
    st.dataframe(merge_origin_df)
    ...
    with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
        ...
        merge_origin_df.to_excel(writer, index=False)
    ...
    st.warning("You must upload file first to see the data.")
